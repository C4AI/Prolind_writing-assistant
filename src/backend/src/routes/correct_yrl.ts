...
export const correctYrl = async (req: Request, res: Response) => {
  ...
  if (
    data.languages["Nheengatu"]["ortographies"][ortography]["spell_checker"] ===
    undefined
  ) {
    res.status(400);
    res.send();
  ...
  try {
    try {
      ...
      let correctedSentenceYrlArray: string[][] = correctedSentenceYrl.slice(
        1
      ) as [];
      ...
      let correctedSentenceYrlArrayFixed: string[][] = [];

      correctedSentenceYrlArray.map((array, index) => {
        if (
          array[0].trim().toLowerCase() === words[index].trim().toLowerCase()
        ) {
      ...
      correctedSentenceYrlArrayFixed = correctedSentenceYrlArrayFixed.map(
        (array) =>
          array.map((word) => {
