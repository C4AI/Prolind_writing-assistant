...
describe("config", () => {
    ...
    test("should update all data properties", async () => {
        ...
        for (let dest_char in dataMocked.char_map) {
            const sourceCharList = dataMocked.char_map[dest_char];
        ...
        expect(data.spellCheckerTimeout).toStrictEqual(dataMocked.spell_checker_timeout);
        expect(data.translatorTimeout).toStrictEqual(dataMocked.translator_timeout);
        expect(data.enableModelChange).toStrictEqual(dataMocked.enable_model_change);
